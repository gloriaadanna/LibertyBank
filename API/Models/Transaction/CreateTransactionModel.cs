...
public class CreateTransactionModelValidator : AbstractValidator<CreateTransactionModel>
{
    public CreateTransactionModelValidator()
    {
        ...
        RuleFor(m => m.SourceAccountNumber).NotEmpty().Length(10);
        RuleFor(m => m.SourceBank).NotEmpty().EmailAddress();
        RuleFor(m => m.DestinationAccountNumber).NotEmpty().Length(10).When(p => p.DestinationAccountNumber is not null);
        RuleFor(m => m.TransactionDate).IsInEnum();
